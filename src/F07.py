...
def finventory(user_data:list, username:str, user : int, monsterinv_list : list, user_monster : list,monster_invent : list,storage : list, user_inventory : list ):
   # user_id = 1
   # monster_id = 2
    ...
    item_monster = testloader.filter_monster(monsterinv_list, user, user_monster)
    # search monster inventory by user id
    items_monster_inventory = testloader.monster_inventory(monsterinv_list, user_monster, monster_invent)
    ...
    for i in items_monster_inventory :
        ...
        no_urut += 1
     # search monster inventory by user id
    items_inventory = testloader.filter_item(storage, user, user_inventory)
...
# menampilkan menu ----------------------------------------------------------------------------------------------
    ...
    # ---------------------------------------
        if type_inv == "monster" :
            res_monster = testloader.filter_monster(monsterinv_list, user, user_monster)
            print_monster +=f'{no_urut}. Monster (Name : {res_monster[1]}, Lvl : , Hp : {res_monster[4]})'
    
        elif type_inv == "potion" :
            res_potion = testloader.filter_item(storage, user, user_inventory)
