...
def RNG(x, y):
    ...
    if x == 1:
        ...
        # Supaya output ada di rentang nilai x sampai y
...
def maks(daftar):
    ...
    # Mulai dari indeks kedua karena kita sudah menganggap indeks pertama sebagai nilai maksimum awal
...
def get_valid_username():
    while True:
        ...
        if cek_karakter(username):
            ...
            print(
                "Username hanya boleh dibentuk oleh alfabet, angka, nomor, strip, dan underscore!")
...
def register(user):
    ...
    if user:  # Jika user telah login
        ...
        for i in range(1, len(user_data)):
            if str(RNG(1, 100)) not in user_data[i][0]:
                new_user.append(str(RNG(1, 100)))
        ...
        tulis_csv('user.csv', user_gabung)
  ...
  """
    ...
    for item in monster_shop_data[1:]:  # Skip header row
        ...
        if monster_id in monster_info:
            ...
            print(f"{monster_id:<2} | {monster_details[0]:<12} | {monster_details[1]:<9} | {
                  monster_details[2]:<9} | {monster_details[3]:<4} | {stock:<4} | {price:<5} | ")
...
def beli_shop():
    ...
    if (beli == "monster"):
        ...
        if (int(monster_shop_data[index][1]) > 0):
            ...
            for monster_entry in monster_inventory[1:]:
                if int(monster_entry[0]) == int(user_id) and monster_entry[1] == id_monster:
                    print(
                        f"Monster {monster_name} sudah ada dalam inventory-mu! Pembelian dibatalkan.")
                    return

            print(f"Berhasil membeli item: {
                  monster_name}. Item sudah masuk ke inventory-mu!")
            monster_inventory.append([user_id, id_monster, 1])
            monster_shop_data[index][1] = str(
                int(monster_shop_data[index][1]) - 1)

            for user_entry in user_data[1:]:
                if user_entry[1] == username_login:
                    user_entry[4] = str(
                        int(user_entry[4]) - int(monster_shop_data[index][1]))
        ...
        if int(item_shop[index][1]) >= jumlah:
            print(f"Berhasil membeli item: {jumlah} Potion of {
                  potion_fixed[index-1]}. Item sudah masuk ke inventory-mu!")
            ...
            if not found:
                item_inventory.append(
                    [user_id, potion_fixed[int(id_potion)-1], str(jumlah)])
    ...
    """
        ...
        for monster in monster_data[1:]:
            if monster[0] not in id_monster_shop:
                print(f"{monster[0]:<2} | {monster[1]:<11}  | {
                      monster[2]:<9} | {monster[3]:<9} | {monster[4]:<4} |")
        ...
        print(monster_data[monster_index][1],
              "berhasil ditambahkan ke dalam shop!")
        ...
        for potion_index in range(len(potion_fixed)):
            # Dapatkan jenis potion berdasarkan indeks
            potion_type = potion_fixed[potion_index]
...
def ubah():
    ...
    if (ubah.lower() == "monster"):
        ...
        if stok_baru and harga_baru:
            ...
            print(monster_name, "telah berhasil diubah dengan stok baru sejumlah",
                  stok_baru, " dan dengan harga baru", harga_baru, "!")
            tulis_csv("monster_shop.csv", monster_shop_data)

        elif stok_baru:
            monster_shop_data[index][1] = stok_baru
            print(
                monster_name, "telah berhasil diubah dengan stok baru sejumlah", stok_baru, "!")
            tulis_csv("monster_shop.csv", monster_shop_data)

        elif harga_baru:
            monster_shop_data[index][2] = harga_baru
            print(monster_name, "telah berhasil diubah dengan harga baru",
                  harga_baru, "!")
            tulis_csv("monster_shop.csv", monster_shop_data)
        ...
        if stok_baru and harga_baru:
            ...
            print(potion_shop_data[index][0], "telah berhasil diubah dengan stok baru sejumlah",
                  stok_baru, " dan dengan harga baru", harga_baru, "!")
            tulis_csv("item_shop.csv", potion_shop_data)

        elif stok_baru:
            potion_shop_data[index][1] = stok_baru
            print(potion_shop_data[index][0],
                  "telah berhasil diubah dengan stok baru sejumlah", stok_baru, "!")
            tulis_csv("item_shop.csv", potion_shop_data)

        elif harga_baru:
            potion_shop_data[index][2] = harga_baru
            print(potion_shop_data[index][0],
                  "telah berhasil diubah dengan harga baru", harga_baru, "!")
            tulis_csv("item_shop.csv", potion_shop_data)
...
def hapus():
    ...
    if (hapus.lower() == "monster"):
        ...
        print("Apakah Anda yakin ingin menghapus",
              monster_name, "dari shop (y/n)? ", end="")
        hapus_confirm = input()
        if (hapus_confirm.lower() == 'y'):
            print(monster_name, "telah berhasil dihapus dari shop!")
            monster_shop_data = monster_shop_data[:index] + \
                monster_shop_data[index+1:]
            tulis_csv("monster_shop.csv", monster_shop_data)
        ...
        if index < 1 or index >= len(potion_shop_data):
            ...
            return
        print("Apakah Anda yakin ingin menghapus",
              potion_shop_data[index][0], "dari shop (y/n)? ", end="")
        hapus_confirm = input()
        if (hapus_confirm.lower() == 'y'):
            print(potion_shop_data[index][0],
                  "telah berhasil dihapus dari shop!")
            potion_shop_data = potion_shop_data[:index] + \
                potion_shop_data[index+1:]
            tulis_csv("item_shop.csv", potion_shop_data)
        else:
            print("Penghapusan dibatalkan.")
...
def lihat_monster():
    ...
    print("ID | Type         | ATK Power | DEF Power | HP   |")
    for monster in monster_data[1:]:  # Skip header row
        print(f"{monster[0]:<2} | {monster[1]:<12} | {
              monster[2]:<9} | {monster[3]:<9} | {monster[4]:<4} | ")
...
def tambah_monster():
    ...
    if (hapus_confirm.lower() == 'y'):
        ...
        monster_data.append(
            [monster_id_baru, monster_name, atk_power, def_power, hp])
        ...
        for monster_entry in monster_inventory[1:]:
            if int(monster_entry[0]) == int(user_id):
                ...
                if found:
                    ...
                    print(f"{i}. {monster_name} (ID: {
                          monster_id}) (Level: {monster_level})")
                i += 1
                found = False
            ...
            for monster_entry in monster_inventory[1:]:
                if int(monster_entry[0]) == int(user_id) and upgrade_monster_no == monster_entry[1]:
                    ...
                    if monster_level == 5:
                        ...
                        print(
                            f"{monster_name} akan di-upgrade ke level {next_level}")
                        print(f"Harga untuk melakukan upgrade {monster_name} adalah {
                              upgrade_price[monster_level]} OC")
                        ...
                        if upgrade_confirm == "Y":
                            ...
                            if upgrade_price[monster_level] <= user_oc:
                                ...
                                monster_level += 1
                                print(f"Selamat, {
                                      monster_name} berhasil di-upgrade ke level {monster_level}!")
                  ...
                  """
                                tulis_csv("user.csv", user_data)
                                tulis_csv("monster_inventory.csv",
                                          monster_inventory)
    ...
    if play_confirm == "Y":
        if 400 <= user_oc:
            ...
            if item1 == item2 and item1 == item3:
                ...
                print(f"JACKPOT!!! Selamat, Anda mendapatkan monster {
                      reward_monster}.")
